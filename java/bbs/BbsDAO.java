...
public class BbsDAO {
	...
	public void upAnsnum(Map map) {
		...
		sql.append(" where grpno = ? and ansnum > ? "); // 부모의 글의 답변을 한거가 올라오는 거므로 부모다 커야 한다?
	...
	public boolean passCheck(Map map) {
		...
		try {
			...
			if(cnt>0)flag =true; // 비번이 맞으면 실행된다.
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}finally {
	...
	public int total(Map map) { //col,word
		...
		String col = (String)map.get("col"); // 선택한다, 검색컬럼: wname,title,content,title_content
		String word = (String)map.get("word"); //사용자 직접 입력한 단어: 이, 2
	...
	public boolean create(BbsDTO dto) {
		...
		sql.append(" values(?,?,?,?,sysdate(), (select ifnull(max(grpno),0)+1 from bbs b) ) "); // ?표는 ''으로 감싸지 않기
